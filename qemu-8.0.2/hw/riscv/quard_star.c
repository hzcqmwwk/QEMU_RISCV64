...
static void quard_star_cpu_create(MachineState *machine)
{
    ...
    {
        error_report("number of sockets/nodes should be less than %d", QUARD_STAR_SOCKETS_MAX);
        exit(1);
    }
    ...
    for (i = 0; i < riscv_socket_count(machine); i++)
    {
        ...
        object_property_set_uint(OBJECT(&s->soc[i]), "resetvec", quard_star_memmap[QUARD_STAR_MROM].base, &error_abort);
...
static void quard_star_memory_create(MachineState *machine)
{
    ...
    {
        error_report("DRAM size is 0; please specify -m, e.g. -m 1G");
        exit(1);
    }
    
    memory_region_init_ram(dram_mem, NULL, "riscv_quard_star_board.dram", machine->ram_size, &error_fatal);
    memory_region_add_subregion(system_memory, quard_star_memmap[QUARD_STAR_DRAM].base, dram_mem);

    memory_region_init_ram(sram_mem, NULL, "riscv_quard_star_board.sram", quard_star_memmap[QUARD_STAR_SRAM].size, &error_fatal);
    memory_region_add_subregion(system_memory, quard_star_memmap[QUARD_STAR_SRAM].base, sram_mem);
...
static void quard_star_machine_init(MachineState *machine) // 初始化各种硬件
{
    ...
    quard_star_rtc_create(machine);    // 创建RTC
    quard_star_serial_create(machine); // 创建UART
    // 创建其他硬件
}
