...
enum {
    QUARD_STAR_UART0_IRQ = 10,  //定义串口中断号为10
...
#define QUARD_STAR_PLIC_NUM_PRIORITIES 7         // PLIC支持的中断优先级的数量
#define QUARD_STAR_PLIC_PRIORITY_BASE  0x04      // PLIC中断优先级寄存器的基址偏移值，用于访问中断优先级信息
#define QUARD_STAR_PLIC_PENDING_BASE   0x1000    // PLIC中断挂起寄存器的基址偏移值，用于访问中断挂起状态
#define QUARD_STAR_PLIC_ENABLE_BASE    0x2000    // PLIC中断使能寄存器的基址偏移值，用于控制中断使能状态
#define QUARD_STAR_PLIC_ENABLE_STRIDE  0x80      // PLIC中断使能寄存器之间的地址间隔
#define QUARD_STAR_PLIC_CONTEXT_BASE   0x200000  // PLIC上下文保存寄存器的基址偏移值，用于保存中断处理程序的上下文信息
#define QUARD_STAR_PLIC_CONTEXT_STRIDE 0x1000    // PLIC上下文保存寄存器之间的地址间隔
#define QUARD_STAR_PLIC_SIZE(__num_context) (QUARD_STAR_PLIC_CONTEXT_BASE + (__num_context) * QUARD_STAR_PLIC_CONTEXT_STRIDE)
